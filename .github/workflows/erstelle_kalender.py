...
def create_calendar_html():
    ...
    try:
        ...
        for component in cal.walk('VEVENT'):
            ...
            try:
                ...
                def add_event_to_week(start_dt, end_dt):
                    ...
                    # Enddatum bei Ganztagsereignissen (exklusiv) anpassen
                ...
                if 'RRULE' in component:
                    ...
                    for occ_start_dt in rrule.between(start_of_week_dt, search_end_dt, inc=True):
                        add_event_to_week(occ_start_dt, occ_start_dt + duration)
                else:
        ...
        # ---------- HTML (fixes 5-Spalten-Layout, kein Responsive/Print) ----------
        html_parts = []
        html_parts.append(f"""<!DOCTYPE html>
<html lang="de">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=1920, initial-scale=1"> <!-- Fix für TV -->
...
.logo {{
  background:#fff; border-radius:10px; padding:6px; display:flex; align-items:center; justify-content:center;
}}
...
.grid {{
  display: grid;
  grid-template-columns: repeat(5, 1fr); /* Fix: genau 5 Spalten */
  gap: 12px;
}}
.day {{
  background: var(--card);
